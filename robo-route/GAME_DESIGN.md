...
This keeps the main pleasure of Human Resource Machine — first come up with a program, then watch it run — but removes the text, the office theme and the difficult abstractions.
...
A new command appears only after a level where the child has already mastered the previous set of actions. Six dots on the field show the progress without numbers; unlocked levels can be replayed.
