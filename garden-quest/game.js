...
// Everything below draws the garden. The board, the levels and the rules above
// it are the model, and test-game.js checks them without a browser.
if (typeof document !== "undefined") {
  ...
  function renderLevelPicker() {
    // The buttons are built anew, so a garden chosen with the keyboard would
    // leave the focus on nothing. It goes to the garden that is open now.
    const hadFocus = levelPickerElement.contains(document.activeElement);
    ...
    if (hadFocus) {
      levelPickerElement.querySelector(`.level-button[data-level="${currentLevelIndex}"]`)?.focus();
    }
  }
